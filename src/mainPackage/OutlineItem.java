...
public class OutlineItem {
	...
	public static void remove(List<OutlineItem> nodesToRemove) {
		...
		List<OutlineItem> nodesToFire = new ArrayList<>();
		
		for (OutlineItem nodeToRemove : nodesToRemove) {
			if (uniqueParentNodes.add(nodeToRemove.parent)) {
				nodesToFire.add(nodeToRemove);
			}
		}
	...
	public List<OutlineItem> getPhotos(OutlineItem item, Pattern[] exclusionPatterns) {
		...
		if (!exclude) {
			for (OutlineItem childItem : item.children) {
				if (!childItem.photoExclusionHit(exclusionPatterns)) {
					searchResults.addAll(getPhotos(childItem, exclusionPatterns));
				}
			}
		}
	...
	public List<OutlineItem> getPhotos(List<OutlineItem> items, Pattern[] exclusionPatterns) {
		...
		for (OutlineItem item : items) {
			if (!item.photoExclusionHit(exclusionPatterns)) {
				searchResults.addAll(getPhotos(item, exclusionPatterns));
			}
		}
